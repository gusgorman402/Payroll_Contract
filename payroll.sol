...
contract workForce
{
    ...
    address antAddr = 0x960b236A07cf122663c4303350609A66A7B288C0;
    address usdAddr = 0x0D50440f844e7d741d2E851d011f839974AF428D;

    ERC20 antToken = ERC20(antAddr);
    ERC20 usdToken = ERC20(usdAddr);
    /* set to 1 Ether equals 275.00 USD */
    uint etherUsdRate = 27500;

    function workForce() public
    {
        owner = msg.sender;
        creationDate = now;
        exchangeRates[antAddr] = 2000;
    }
    ...
    function calculatePayrollRunway() constant onlyOwner returns (uint)
    {
        ...
        uint allTokenBalance = tokenUsdAmount( antAddr, antToken.balanceOf(address(this)));
        allTokenBalance += etherUsdAmount( this.balance );
        allTokenBalance += usdToken.balanceOf(address(this));
        
        uint daysRemaining = allTokenBalance / dailyPayout;
    ...
    function setPercentTokenAllocation(uint _usdTokens, uint _ethTokens, uint _antTokens) onlyEmployee
    {
        ...
        if( now < workcrew[x].lastTokenConfigDay + 24 weeks ){revert;}
    ...
    {
        uint x = employeeIdIndex[_employeeId];
        if( now < workcrew[x].lastPayday + 4 weeks ){ revert; }
        if( msg.sender != workcrew[x].employeeAddress ){ revert; }
        workcrew[x].lastPayday = now;

        uint paycheck = workcrew[x].yearlySalaryUSD / 12;
    ...
    function setExchangeRate(address _token, uint _UsdValue) onlyOracle
    {
        exchangeRates[_token] = _UsdValue;
    }

    function setEtherExchangeRate(uint _usdValue) onlyOracle
    {
        etherUsdRate = _usdValue;
    }

    function etherUsdAmount(uint _etherAmount) constant returns (uint)
    {
        uint retVal = _etherAmount * etherUsdRate;
        return retVal;
    }

    function tokenUsdAmount(address _token, uint _tokenAmount) constant returns (uint)
    {
        uint usdAmount = _tokenAmount * exchangeRates[_token];
        return usdAmount;
    }
}
